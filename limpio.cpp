...
struct NodoArchivo {
        ...
        siguienteHermano(NULL) // Inicializa siguiente hermano como nulo
    {
        ...
        if (padre == NULL) {
        rutaAbsoluta = "/";  // La raíz siempre es "/"
    } else {
        // Evitar dobles barras al concatenar
        if (padre->rutaAbsoluta.back() == '/') {
            rutaAbsoluta = padre->rutaAbsoluta + nombre;
        } else {
            rutaAbsoluta = padre->rutaAbsoluta + "/" + nombre;
        }
    }
        
        if (esDirectorio && rutaAbsoluta.back() != '/') {
            rutaAbsoluta += "/";
        }
    }
...
NodoArchivo* directorioActual = NULL;
...
string* dividirRuta(const string& ruta, int& cantidadElementos) {
    ...
    if (cantidadElementos == 0) return NULL;     // Si no hay elementos, retorna nulo
...
bool crearDirectorio(const string& nombre) {
    if (!directorioActual || !directorioActual->esDirectorio) {
        cout << "ERROR No se puede crear en la ubicación actual" << endl;
        return false;
    }
    
    // Validación mínima (solo nombre no vacío)
    if (nombre.empty()) {
    ...
    if (buscarHijo(directorioActual, nombre) != NULL) {
        cout << "ERROR Ya existe '" << nombre << "' en este directorio" << endl;
        return false;
    }
    
    /////////////////////////////////////PREGUNTAR A GABRIELA!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
    ...
    NodoArchivo* nuevoDir = new NodoArchivo(nombre, true, directorioActual);
    agregarHijo(directorioActual, nuevoDir);
...
void eliminarArchivoODirectorioRecursivo(NodoArchivo* nodo) {
    if (nodo == NULL) return;

    // Primero eliminar todos los hijos (si es directorio)
    if (nodo->esDirectorio) {
        NodoArchivo* hijoActual = nodo->primerHijo;
        while (hijoActual != NULL) {
            NodoArchivo* siguiente = hijoActual->siguienteHermano;
            eliminarArchivoODirectorioRecursivo(hijoActual);
...
// Función para eliminar un archivo o directorio (recursivamente)
bool eliminarArchivoODirectorio(const string& nombre) {
    ...
    // Buscar el nodo a eliminar entre los hijos del directorio actual
    NodoArchivo* nodoAEliminar = NULL;
    NodoArchivo* anterior = NULL;
    NodoArchivo* hijo = directorioActual->primerHijo;
    
    // Buscar el nodo y mantener referencia al anterior para reenlazar
    while (hijo != NULL) {
    ...
    // Eliminar recursivamente si es un directorio con contenido
    if (nodoAEliminar->esDirectorio && nodoAEliminar->primerHijo != NULL) {
        cout << "Eliminando directorio '" << nombre << "' y todo su contenido..." << endl;
        
        // Primero eliminar todos los hijos
        NodoArchivo* hijoActual = nodoAEliminar->primerHijo;
        while (hijoActual != NULL) {
            NodoArchivo* siguiente = hijoActual->siguienteHermano;
            eliminarArchivoODirectorioRecursivo(hijoActual);
            hijoActual = siguiente;
    ...
    // Reenlazar la lista de hermanos
    if (anterior == NULL) {
        // Era el primer hijo
        directorioActual->primerHijo = nodoAEliminar->siguienteHermano;
    } else {
        anterior->siguienteHermano = nodoAEliminar->siguienteHermano;
    }
    
    if (nodoAEliminar->esDirectorio) {
...
bool crearArchivo(const string& nombre) {
    if (!directorioActual || !directorioActual->esDirectorio) {
    ...
    // Validar que el nombre contenga al menos un punto (para ser considerado archivo)
    if (nombre.find('.') == string::npos) {
        cout << "ERROR: El nombre debe contener una extensión (ej: archivo.txt)" << endl;
        return false;
    }

    // Crear el nuevo archivo (no es directorio)
    NodoArchivo* nuevoArchivo = new NodoArchivo(nombre, false, directorioActual);
    agregarHijo(directorioActual, nuevoArchivo);
...
int main() {
    ...
    if (!cargarDesdeArchivo("Ejemplo1.txt")) {
        ...
        cout << "             Recuerde aregarlo al output\n" << endl;
        return 1;
    }
    ...
    while (true) {
        ...
        // Comando para salir
            ...
            cout << "  touch  - Crear archivo tetxo plano" << endl;
